...
import { Mail, QrCode, Headset } from 'lucide-react';
import { useLanguage } from '@/contexts/language';

const PlansHeroSection = () => {
  const { t } = useLanguage();
  const h = t.plansPage.hero;

  return (
    <section className="bg-gradient-to-br from-white via-gray-50 to-palop-green/5 py-14 md:py-20">
      <div className="container mx-auto px-4">
        <div className="max-w-2xl mx-auto text-center space-y-6">
          <p className="text-sm font-semibold text-palop-green uppercase tracking-wider" data-testid="plans-hero-eyebrow">
            {h.eyebrow}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center pt-1">
            ...
            <Button
              ...
              data-testid="plans-hero-secondary-cta"
            >
              <Link to="/esim">{h.secondaryCta}</Link>
            </Button>
          </div>

          <div className="flex flex-wrap gap-4 justify-center pt-2">
            {[
              { icon: Mail, label: h.trust1 },
              { icon: QrCode, label: h.trust2 },
